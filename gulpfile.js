...
/**
 * Build the Jekyll Site - for windos. If you are on a Mac/linux change jekyll to just jekyll
 */
gulp.task('jekyll-build', function (done) {
	return cp.spawn('jekyll', ['build'], {stdio: 'inherit', cwd: 'site'}).on('close', done);
});
...
gulp.task('css-min', function () {
	return gulp.src([dist_path + '/css/**/*.css', '!' + dist_path + '/css/**/*.min.css'])
		.pipe(sourcemaps.init())
		.pipe(csso())
...
gulp.task('vendors', gulp.series('clean-vendors',
	function () {
		return gulp.src('./site/_assets/vendors/**/*')
			.pipe(gulp.dest(dist_path + '/vendors'));
	}, function () {
		return gulp.src('./js/bootstrap/**/*')
			.pipe(gulp.dest(dist_path + '/vendors/bootstrap'));
	}));
...
gulp.task('serve', gulp.parallel('jekyll-build', function (done) {
	return cp.spawn('jekyll', ['serve'], {stdio: 'inherit', cwd: 'site'}).on('close', done);
}));
...
gulp.task('imagemin', function () {
	return gulp.src([dist_path + '/images/**/*.jpg', dist_path +'/images/**/*.png'])
		.pipe(imagemin())
